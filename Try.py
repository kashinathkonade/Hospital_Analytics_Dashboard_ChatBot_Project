...
# Function to merge multiple CSV files while handling duplicate columns
def merge_csv_files(uploaded_files):
    try:
        ...
        for file in uploaded_files:
            ...
            df.columns = pd.io.parsers.read_csv(file).columns  # Ensure unique columns

            dataframes.append(df)

        # Merge intelligently on common keys
        common_keys = set.intersection(*(set(df.columns) for df in dataframes))
        if common_keys:
            merge_column = list(common_keys)[0]
            merged_df = dataframes[0]
            for df in dataframes[1:]:
                merged_df = pd.merge(merged_df, df, on=merge_column, how="outer", suffixes=("_left", "_right"))
        else:
            merged_df = pd.concat(dataframes, axis=1, ignore_index=False, sort=False)
...
if st.session_state.selected_page == "Chatbot":
    ...
    if uploaded_files:
        ...
        if merged_csv_path:
            ...
            st.session_state.csv_path = merged_csv_path
            st.session_state.merged_df = merged_df

            # Show Merged Data Preview
            st.write("📜 **Preview of Merged CSV Data:**")
            st.dataframe(merged_df.head())

    # Ensure CSV file is loaded
    ...
    dashboard_url = "https://app.powerbi.com/view?r=YOUR_POWERBI_DASHBOARD_LINK"
    st.markdown(f"""
